...
BASE_OUTPUT_DIR = "./gemma_soft_prompt"
...
try:
    ...
    logger.info(f"  Virtual tokens: {CONFIG['virtual_tokens']}")
    ...
    train_val = split["train"].train_test_split(test_size=0.1111, seed=42)  # 0.1111 ≈ 1/9
    dataset = {
    ...
    logger.info(
        f"Dataset split - Train: {len(dataset['train'])}, "
        f"Validation: {len(dataset['validation'])}, "
        f"Test: {len(dataset['test'])}"
    )

    logger.info("=== PHASE 8: Training Setup ===")
    OUTPUT_DIR = os.path.join(BASE_OUTPUT_DIR, f"gemma2b_it_prompt_tuning_vt{CONFIG['virtual_tokens']}_{session_timestamp}")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ...
    total_steps = len(dataset["train"]) * CONFIG["num_epochs"] // effective_batch_size
